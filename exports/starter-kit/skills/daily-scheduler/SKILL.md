...
description: Plan, update, or view the daily schedule. Always run schedule_data.py first. v2 separates PLAN (intent) from ACTUAL (truth), supports cross-midnight, and syncs ACTUAL to GCal safely.
---
...
- Calendar sync follows **ACTUAL**, never PLAN.

## Mandatory first step
Before planning/replanning/viewing:
```bash
...
- `memory/schedules/.meta/YYYY-MM-DD.json` — sync IDs / locks / hashes
- `memory/schedules/.archive/YYYY-MM-DD/<timestamp>.md` — backup before writes
...
- Cross-midnight:
  - `- [A] 23:45-01:30(+1d) Late Work {uid:A-..., ps:exact, pe:exact, status:done}`
- Approx:
  - `- [A] ~17:30-18:10 Exercise {uid:A-..., ps:approx, pe:exact, status:done}`
...
6) **Sync**
- Sync **ACTUAL timeline only** to external calendar.
...
2. Managed events must include `daily-scheduler/v2` and `daily-scheduler uid=A-...`
...
- Cross-midnight normalized
- ACTUAL timeline sorted by start
- If uncertain, preserve raw text in `## Inbox`

## Customization
See `memory/scheduling-rules.md` for user-specific routing preferences.
Default order: schedule file → Google Calendar → Todoist.
Never delete past events unless user explicitly asks.
