...
description: >
  ...
  (5) user says "remember this", "learn from this", "don't do that again", (6) debugging/RCA needed.
...
- Format: `area.specific_issue` (e.g., `config.direct_edit_fails`, `path.parent_levels`)
...
When debugging: Reproduce → Isolate → Hypothesize → Fix → Record (via `learn.py error`).
