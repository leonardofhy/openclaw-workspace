...
Usage:
    ...
    # Log an income event
    python3 finance_snapshot.py --log-income --source tutoring --amount 4800 [--note "2 sessions"]
...
def cmd_log_income(source: str, amount: float, note: str = ""):
    """Log an income event."""
    ...
    item = store.append(entry)
    print(f"✅ Income logged: {item['id']} | {source} | TWD {amount:,.0f} | {today}")
...
def main():
    ...
    parser.add_argument("--source", type=str, help="Income source name")
    parser.add_argument("--amount", type=float, help="Income amount (TWD)")
    ...
    if args.latest:
        ...
        cmd_log_income(args.source, args.amount, args.note)
