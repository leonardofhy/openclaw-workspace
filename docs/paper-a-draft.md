...
1. **gc(k) metric.** The first causal, layer-wise grounding coefficient for audio-language models, based on interchange intervention that quantifies audio dependency at each layer. Unlike observational probes (AudioLens) or necessity-only tests, gc(k) provides both sufficiency and necessity evidence for audio grounding.
2. **Listening Geometry.** A 5D framework (k*, α_AND, σ, t*, CS) that taxonomizes ALM listening strategies into four profiles: strong listeners, shallow listeners, sophisticated guessers, and fragile listeners. Enables systematic comparison across models and deployment conditions.
3. **AND/OR gate decomposition.** A mechanistic taxonomy of multimodal features distinguishing genuine audio-text integration (AND-gates) from modality-redundant processing (OR-gates). Provides the first causal test of multimodal feature dependence in speech models.
4. **Mock framework validation.** 29 experiments total: 27 mock validation experiments (median |r| = 0.877) demonstrate internal consistency, plus 2 real experiments providing preliminary evidence from Whisper-base encoder voicing geometry at layer 5 [TODO: fill in from real data].
5. **Pre-registered predictions.** 3 testable claims for scale-up and RL-training experiments: ALME conflict items should show late-layer gc drop, rare phoneme contrasts should be more vulnerable to text override, and degraded audio should flatten gc across all layers.
