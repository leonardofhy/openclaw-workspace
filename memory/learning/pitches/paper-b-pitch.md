...
**§2.4 — SAEBench Metric Portability to Audio (added v1.7, cycle #302)**

We systematically assess which of SAEBench's eight evaluation categories port to audio with low modification, which require audio-native design work, and which have no audio analogue and must be replaced. This analysis positions AudioSAEBench as a principled extension, not an ad hoc collection.

**Directly portable (LOW complexity, ≤1 day implementation):** Four SAEBench categories apply to audio with minimal change. Reconstruction Fidelity (SAEBench M1) measures cosine similarity between original and SAE-reconstructed activations at the token level; the audio analogue operates at the 30ms frame level, with identical mathematics and lower temporal granularity than text. L0 Sparsity (M2) counts active features per token in text; in audio, this becomes features per 30ms frame, with a natural phoneme-conditional extension — sparsity should be lower for silence and simple phonemes than for complex clusters such as /str/ or /spl/, providing a diagnostic richer than the text analogue. Downstream Task Performance (M6) in text measures probe accuracy on NLP tasks from SAE latents; in audio it decomposes into two complementary measures: phoneme label prediction from SAE latents (analogous to POS-tag probing in text), and SAE-reconstruction WER — the fraction of transcription accuracy preserved when Whisper's internal activations are routed through the SAE bottleneck. Feature Geometry (M8) analyzes cosine similarity structure between features and checks for superposition via anti-podal pairs; in audio, the natural grouping is by phonological class (stop/fricative/nasal/vowel), with the prediction that voiced/unvoiced features form anti-podal pairs (cosine $\approx -1.0$) consistent with binary acoustic distinctions.

**Conceptually portable with moderate adaptation (MEDIUM complexity, 2–3 days):** Three categories require conceptual translation but retain the same evaluative goal. The Absorption Score (M3) in text measures when one SAE feature "absorbs" the semantic function of another; in audio, this maps to a gc(k) Absorption Test — in audio-dominant frames (gc(k) $> 0.8$, from Paper A's layer-level grounding analysis), we test whether non-acoustic SAE features anomalously co-activate, indicating that a semantic slot has absorbed the audio-grounding signal. Interventional Selectivity (M5) in text ablates a feature and measures downstream logprob delta; in audio, the analogous signal is WER delta on Whisper's CTC output, with a local/global decomposition: WER change restricted to frames where the ablated feature was active (local effect) versus WER change across the full utterance (global effect), yielding a selectivity ratio that distinguishes targeted phoneme-level interventions from diffuse disruptions. Spurious Correlation (M7) in text checks whether a feature activates for semantically unrelated tokens; in audio, the test is cross-phoneme activation overlap — whether feature $f$ fires for phonemes that are acoustically and articulatorily unrelated. Concretely, feature $f$ is extracted, its top-decile frames are phoneme-labeled via MFA alignment, and a $\chi^2$ test evaluates whether the observed phoneme distribution is consistent with chance activation. A well-structured audio SAE feature should activate for acoustically proximate phonemes (e.g., the bilabial cluster /p/, /b/, /m/) rather than phonemes chosen uniformly at random.

**Requires novel audio-native design (HIGH complexity, ≥1 week):** One category has no text equivalent and constitutes AudioSAEBench's primary conceptual contribution. The Monosemanticity / RAVEL Isolate metric (M4) in text uses the RAVEL benchmark (Huang et al., ACL 2024), which evaluates SAE features using entity–attribute minimal pairs drawn from text corpora, applying Cause/Isolate scoring to determine whether patching feature $F$ changes attribute $A$ without collateral damage to other attributes. The audio analogue must confront a deeper challenge: acoustic attributes co-occur at the physical signal level in training corpora in ways that lexical attributes do not. Voicing co-occurs with speaker gender distributions; spectral noise co-occurs with emotional arousal; pitch variation co-occurs with affect and syntactic prominence. This structural co-occurrence drives audio SAE features to encode multiple phonological attributes simultaneously regardless of training procedure, making the Isolate score systematically lower for audio than for text — not a training failure but a representational inevitability. AudioSAEBench's Category 0 (Audio-RAVEL) addresses this using phonological minimal-pair stimuli from Choi et al. (arXiv:2602.18899, 96 languages) as the intervention stimuli: pairs differing only in a single phonological feature (voicing, manner, or place of articulation) enable controlled measurement of Cause and Isolate scores. ACES (Parekh et al., arXiv:2603.03359) independently confirms that accent features are "deeply entangled with recognition-critical cues" and that linear subspace attenuation does not reduce this entanglement — providing behavioral prior-art evidence that high cross-attribute leakage in audio representations is real and not merely an AudioSAEBench artifact. Audio-RAVEL is therefore both the hardest metric to implement and the one most likely to reveal genuine structural weaknesses in audio SAE design.

**Summary of portability.** Of SAEBench's 8 metrics, 4 port with low modification (M1 Reconstruction, M2 Sparsity, M6 Downstream, M8 Geometry), 3 require audio-specific adaptation (M3 Absorption, M5 Selectivity, M7 Spurious), and 1 — the disentanglement metric (M4) — requires a fully novel stimulus design and framing (Audio-RAVEL / PCDS). AudioSAEBench also adds two metrics that have no SAEBench text analogue: Temporal Coherence Score (TCS) in Category 1, which exploits audio's inherent temporal structure via phoneme-boundary variance ratios, and Grounding Sensitivity (gc(F)) in Category 5, which tests whether features respond to audio content versus linguistic prediction context. Together, AudioSAEBench inherits the SAEBench multi-metric philosophy while extending it with the evaluation dimensions most critical for audio mechanistic interpretability: isolation from co-occurring acoustic attributes and localization of the audio-versus-text processing boundary.

**Status of §2:** ✅ DRAFT COMPLETE. §2.1-§2.3 = ~700 words, 3 subsections, all cite IDs confirmed. §2.4 (portability prose) = ~600 words, added cycle #302. Total §2 ≈ 1300 words — may need trimming to fit 9-page ACL budget; §2.4 can be condensed to the summary table alone if space-constrained. Anti-bloat check passed: all citations are existing confirmed papers; no new claims. Ready to copy into LaTeX. Structurally parallel to §1 (same theory pentagon, same Pearl hierarchy).
